...
| `/media/[id]` | GET, POST, PATCH, DELETE | bearer (POST/PATCH + `content.update`) | POST: `action:'refresh_insights'` rechecks Photo-insight consent, current caption/tag context, rate limit, and processing budget. PATCH: `customTags` (≤10, ≤30 chars each) **or** `caption` ≤300 — presence of the `customTags` key selects the branch | GET: `kind:'found', media{…, text, transcript, summary, caption, narrative, tags[], customTags[], insights[{label,value}], clarifyingQuestions[] (optional, ≤3, ≤180 chars each), playbackUrl, expiresIn, rendition}`. POST: `kind:'refreshed'|'replayed'|'skipped'|'processing', message`. PATCH: `kind:'updated', media{…}` or `kind:'updated', customTags[]`. DELETE: `deleted:true` |

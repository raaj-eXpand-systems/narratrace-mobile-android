...
## Pre-submission owner review fixes — 23 September 2026

Both native apps now expose localized date and time picker buttons and searchable time-zone selection for Letters and media delivery. Selected zones are sent through the existing delivery contract; future-time validation, recipient safeguards and server authorization remain authoritative. Android rejects nonexistent daylight-saving local times. No Letters were saved, scheduled or sent during screenshot verification.

Both native theme catalogs now match the web's 14 choices (System, Light, Dark and 11 named palettes). Narratrace Blue is the default for an unset preference; explicit saved preferences are preserved. System/Light/Dark remain primary and the named themes are under More. Named palette tokens follow the web, with platform-native controls preserved.

Android replaces device-dependent cursive with bundled OFL-licensed Pinyon Script for visible Nia full names and possessives. Compact N avatars remain sans-serif. iOS keeps its existing signature font. Marin voice and playback behavior are unchanged.

iOS previously mapped broad request failures to offline. Cancellation is now ignored, verified no-network errors remain offline, transient connection errors have separate copy, and service/access failures no longer falsely claim no network transfer was attempted. This fixes a confirmed classification bug; it does not establish the cause of every historical network interruption. Session, entitlement and local-only access restrictions remain enforced.

Primary scoped Security gate PASS: existing authentication, session rotation, access controls, recipient verification and server delivery validation preserved; no migration, environment-variable change, new service or permissions. No private content added to source; approved demonstration data only in captures. Release packages exclude Markdown/MDX/SQL. Code Quality gate PASS: focused contract/classification/theme/DST regression tests, native release builds and diff whitespace checks. Manual simulator/emulator evidence covers Blue, Midnight Archive, theme menus, native date/time controls, and Europe/London selection. Manual acceptance of every theme and physical-device accessibility/audio/background behavior is not claimed.

Impact assessment: both native clients reconciled. Web already provides the authoritative theme catalog and compatible delivery API; no web, marketing, Admin or Expand Systems source change is required. QA/operations impact is this release evidence and remaining device acceptance. No agents or paused QA database work activated. Refreshed screenshots are in the private workspace review-fixes capture folder. No store upload or submission.

Final evidence: 207 unit tests passed, zero failures/errors/skips; lintDebug, verifyStoreRelease, bundleRelease and assembleRelease passed using the existing Doppler configuration. AAB signature verified (standard self-signed upload certificate warning). Version 1.0.0 (3). AAB SHA-256: `e1afcaab0eaa1f995f66ce6ffed4bd4a73e90607b9f38bec25ddc30c6e9e2491`; APK SHA-256: `2b0064ca019b21caffe58bcad421ed3295be3be63a2e1b92f1c534ce02f26048`. Existing app/build/outputs release paths contain these final candidates, superseding earlier 23 September packages.
